<template>
  <div class="posts pt-15" id="posts">
    <h3
      class="text-center text-h5 text-xl-h2 text-md-h3 font-weight-light"
      data-aos="flip-right"
      data-aos-delay="200"
    >
    ...
    <h2
      v-if="posts.length == 0"
      class="text-h5 text-md-h3 text-xl-h2 pt-15 px-10 text-center"
      data-aos="flip-right"
      data-aos-delay="200"
    >
      No hay articulos en este momento
      <v-icon class="no-articles" x-large color="red">
    ...
    <v-container data-aos="flip-left" class="mt-15">
      <v-row>
        <v-col cols="12" md="4" v-for="post in posts" :key="post._id">
          <v-card>
            <v-img max-height="20rem" :src="post.img" />
            <div class="pa-5">
              <h3 class="text-h6">
                {{ post.title }}
              </h3>
              ...
              <p class="font-italic grey--text">
                Fecha: {{ post.date | moment("dddd, MMMM Do YYYY") }}
              </p>
    ...
    <v-container class="d-flex justify-center">
      <router-link to="/create-post" v-if="posts.length < 6">
        <v-btn :disabled="posts.length >= 6" color="green" outlined tile>
          CREA UN ARTICULO
        </v-btn>
      </router-link>
    </v-container>
